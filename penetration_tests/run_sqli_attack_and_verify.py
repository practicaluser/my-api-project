...
from app.models import SecurityEvent # app/models.py에서 SecurityEvent 모델을 직접 가져옵니다.
...
def execute_attack_phase():
    ...
    for scenario in payload_data["scenarios"]:
        ...
        result = {"name": name, "payload": payload, "success": False, "response": "", "technique": technique}
        ...
        try:
            ...
            if technique == "Time-Based":
                if elapsed_time > 4.0:
                    success = True
                    print(f"  [+] SUCCESS: Time-based attack detected. Response time: {elapsed_time:.2f}s")
                else:
                    print(f"  [-] FAILED: No significant time delay detected. Response time: {elapsed_time:.2f}s")
            
            # 2. 오류 기반(Error-Based) 공격 성공 조건: 500 에러 또는 응답에 DB 에러 메시지 포함
            elif technique == "Error-Based":
                if response.status_code == 500 or any(err in response.text.lower() for err in ["sql syntax", "error", "warning", "fatal"]):
                    success = True
                    print(f"  [+] SUCCESS: Error-based attack detected with status {response.status_code}.")
                else:
                    print(f"  [-] FAILED: No database error observed in response.")

            # 3. 유니언 기반(UNION-Based) 공격 성공 조건: 정상 응답(200)이면서, 결과가 1개 초과로 반환 (모든 데이터를 훔쳤다고 가정)
            elif technique == "UNION-Based":
                 try:
                    if response.status_code == 200 and len(response.json().get("results", [])) > 1:
                        success = True
                        print(f"  [+] SUCCESS: UNION-based attack seems successful, returned {len(response.json().get('results',[]))} items.")
                    else:
                        print(f"  [-] FAILED: UNION-based attack did not return expected data.")
                 except json.JSONDecodeError:
                    print(f"  [-] FAILED: Response was not valid JSON, which might indicate an error page.")
            
            # 4. 기타 (Boolean-Based 등) : 일단 200 응답이면 성공으로 간주, 추가 검증 필요
            else:
                if response.status_code == 200:
                    success = True # 일단 성공으로 간주하고, Blue Team에서 로그 등을 확인
                    print(f"  [?] SUCCESS (provisional): Attack sent and received HTTP 200. Manual verification or log check needed.")
                else:
                    print(f"  [-] FAILED: Received unexpected status code {response.status_code}")
            ...
            if technique == "Time-Based" and "Read timed out" in str(e):
                 result["success"] = True
                 print(f"  [+] SUCCESS: Time-based attack detected (Request timed out as expected).")
            else:
...
def execute_verification_phase(attack_results):
    ...
    try:
        ...
        impact_msg = "NOT_APPLICABLE: No data tampering payloads were executed in this test run."
        print(f"  [*] {impact_msg}")
        verification_results["impact_analysis"].append({"check": "Data Tampering", "status": "NOT_APPLICABLE", "details": impact_msg})
        ...
        try:
            ...
            events = db.query(SecurityEvent).filter(SecurityEvent.event_type == 'SQL_INJECTION_ATTEMPT').all()
            ...
            if len(events) >= successful_attacks and successful_attacks > 0:
                ...
                print(f"  [+] {detect_msg}")
                verification_results["detection_verification"].append({"check": "Log Existence", "status": "SUCCESS", "details": detect_msg})
            elif successful_attacks == 0:
                detect_msg = "INFO: No attacks were successful, so no detection logs are expected."
                print(f"  [*] {detect_msg}")
                verification_results["detection_verification"].append({"check": "Log Existence", "status": "INFO", "details": detect_msg})
            else:
                detect_msg = f"FAILURE: Expected at least {successful_attacks} 'SQL_INJECTION_ATTEMPT' events, but found only {len(events)}."
                print(f"  [-] {detect_msg}")
                verification_results["detection_verification"].append({"check": "Log Existence", "status": "FAILURE", "details": detect_msg})
        except Exception as e:
            # security_events 테이블이 없는 경우 등 예외 처리
            detect_msg = f"ERROR: Could not verify detection logs. Reason: {e}"
            print(f"  [!] {detect_msg}")
            verification_results["detection_verification"].append({"check": "Log Existence", "status": "ERROR", "details": detect_msg})
...
"""
    # 최종 결론
    detection_failed = any(r['status'] == 'FAILURE' for r in verification_results['detection_verification'])
