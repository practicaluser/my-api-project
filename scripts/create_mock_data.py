...
def _create_guaranteed_scenarios(db: Session):  # ✅ db: Session 인자 받기
    ...
    for i in range(10):
        ...
        log = AccessLog(
            ...
            details="Guaranteed brute-force scenario.",
        )
        event = SecurityEvent(
    ...
    for i in range(15):
        ...
        log = AccessLog(
            ...
            event_type="SCANNING_ATTEMPT",
            details="Guaranteed scanner scenario.",
        )
        logs_to_add.append(log)
...
def _create_mock_data(
    db: Session, num_logs: int = 1000, anomalies: bool = True
):  # ✅ db: Session 인자 받기
    ...
    for _ in range(num_logs):
        log_time = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(
        ...
        if anomalies:
            ...
            scenario = random.choices(
                ["normal", "sqli_attempt", "permission_denied"], weights=[95, 3, 2], k=1
            )[0]
        else:
        ...
        log = AccessLog(
            timestamp=log_time, response_time_ms=abs(random.gauss(80, 50)) + 10
        )
        if scenario == "normal":
            log.ip_address = ip_addr
            log.path = random.choice(PATHS)
            log.method = random.choice(METHODS)
            log.status_code = random.choices(
                [200, 201, 302, 404, 500], weights=[85, 5, 2, 5, 3], k=1
            )[0]
            log.event_type = "NORMAL"
        elif scenario == "sqli_attempt":
            log.ip_address = sqli_attempt_ip
            log.path = "/search?q=' OR 1=1; --"
            log.method = "GET"
            log.status_code = 400
            log.event_type = "SQL_INJECTION_ATTEMPT"
            log.details = "Random SQL injection attempt."
            events_to_add.append(
                SecurityEvent(
                    event_type="SUSPICIOUS_QUERY",
                    username=username,
                    ip_address=sqli_attempt_ip,
                    timestamp=log_time,
                    description=log.details,
                )
            )
            ...
            log.ip_address = ip_addr
            log.path = "/admin/dashboard"
            log.method = "GET"
            log.status_code = 403
            log.event_type = "PERMISSION_DENIED"
            log.details = f"User '{username}' tried to access admin dashboard."
            events_to_add.append(
                SecurityEvent(
