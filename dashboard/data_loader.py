...
def _check_and_display_secrets_status():
    ...
    if "secrets_status_checked" not in st.session_state:
        if os.path.exists(".streamlit/secrets.toml"):
            st.success(
                "✅ .streamlit/secrets.toml 파일이 존재하며, 성공적으로 로드되었습니다."
            )
            # 파일이 존재한다는 상태를 세션에 저장
            st.session_state["secrets_file_exists"] = True
        else:
            st.error(
                "❌ .streamlit/secrets.toml 파일을 찾을 수 없습니다. 프로젝트 루트에 파일을 생성해주세요."
            )
            # 파일이 없다는 상태를 세션에 저장
...
def load_db_data():
    ...
    try:
        ...
        st.error(
            f"데이터베이스 연결 오류가 발생했습니다. 'secrets.toml'의 DB 이름과 `docker-compose.yml`의 `MYSQL_DATABASE` 값이 일치하는지 확인해주세요. 원본 오류: {e}"
        )
        return None
...
def load_latest_qa_report():
    ...
    try:
        if "GITHUB_TOKEN" not in st.secrets or "GITHUB_REPO" not in st.secrets:
            st.error(
                "secrets.toml 파일에 GITHUB_TOKEN 또는 GITHUB_REPO 키가 없습니다. 파일을 다시 확인해주세요."
            )
        ...
        if not workflow_runs:
            return (
                None,
                None,
                "조건에 맞는 성공한 워크플로우 실행 기록을 찾을 수 없습니다.",
            )

        latest_run = workflow_runs[0]
        latest_run_id = latest_run["id"]
        commit_message = latest_run.get("head_commit", {}).get(
            "message", "커밋 메시지 없음"
        )
        ...
        artifact_info = next(
            (item for item in artifacts if item["name"] == "qa-test-report"), None
        )
        ...
        if not artifact_info:
            return (
                None,
                None,
                "워크플로우 실행에서 'qa-test-report' 아티팩트를 찾을 수 없습니다.",
            )
